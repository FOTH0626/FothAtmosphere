#ifndef MULTI_SCATTERING_HLSL
#define MULTI_SCATTERING_HLSL
...
float3 IntegralMultiScattering(
    in AtmosphereParams param, float3 samplePoint, float3 lightDir,
    Texture2D _transmittanceLut, SamplerState samplerLinearClamp)
{
    ...
    for(int i=0; i<N_DIRECTION; i++)
    {
        ...
        float dis = RayIntersectSphereLength(float3(0,0,0), param.PlanetRadius + param.AtmosphereHeight, samplePoint, viewDir);
        float d = RayIntersectSphereLength(float3(0,0,0), param.PlanetRadius, samplePoint, viewDir);
        ...
        for(int j=0; j<N_SAMPLE; j++)
        {
            ...
            float3 t1 = TransmittanceToAtmosphereByLut(param, p, lightDir, _transmittanceLut, samplerLinearClamp);
            float3 s  = ScatteringOnPoint(param, p, lightDir, viewDir);
...
float3 GetMultiScattering(in AtmosphereParams param, float3 p, float3 lightDir, Texture2D lut, SamplerState spl)
{
    float h = length(p) - param.PlanetRadius;
